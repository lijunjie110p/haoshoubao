	...
	<view class="main">
		<view class="form">
			...
			<u-form :error-type="['toast']" :model="forminfo" ref="uForm">
				...
				<u-form-item label-width="150" label="有效日期" prop="valid_date">
					<u-input type="select" @click="showTime=true" v-model="forminfo.valid_date" placeholder="请选择有效日期"> </u-input>
					<u-picker confirm-color="#FFBA37" :params="timeparms" :safe-area-inset-bottom="true" @confirm="selectedDate" mode='time'
					 :default-time="date" v-model="showTime"></u-picker>
				</u-form-item>
				<u-form-item label-width="150" label="CVV2" prop="cvv2">
					<u-input type="number" maxlength="3" v-model="forminfo.cvv2" placeholder="请输入卡背后三位数"> </u-input>
					<u-icon slot="right" color="#f94b05" size="45" @click="showCvv2=true" name="question-circle"></u-icon>
				</u-form-item>
		...
		<u-popup  :border-radius="20"
		 :safe-area-inset-bottom="true" v-model="showCvv2" mode="bottom">
			...
			<view class="u-padding-20 u-text-center main-bg" @click="showCvv2=false"><text class="u-font-30 white-color">知道了</text></view>
		</u-popup>
			...
			async sub() {
				...
				let res = await this.http.request({
					...
					data: {
						...
						validate_code: this.forminfo.cvv2,
						termofvalidity: this.forminfo.valid_date
					}
				})
